 ...
 * @author Rari team, w/ tweaks from Duality (for non-mainnet deploys)
 */
contract MasterPriceOracle is PriceOracleInterface {
    address public immutable WETH_ADDRESS;

    /**
     * @dev Maps underlying token addresses to `PriceOracle` contracts (can be `BasePriceOracle` contracts too).
     */
    mapping(address => PriceOracleInterface) public oracles;
     ...
     */
    function add(address[] calldata underlyings, PriceOracleInterface[] calldata _oracles) external onlyAdmin {
        // Input validation
        require(
            underlyings.length > 0 && underlyings.length == _oracles.length,
            "Lengths of both arrays must be equal and greater than 0."
        ...
        for (uint256 i = 0; i < underlyings.length; i++) {
            if (!canAdminOverwrite)
                require(
                    address(oracles[underlyings[i]]) == address(0),
    ...
    function getUnderlyingPrice(CTokenInterface cToken) external view override returns (uint256) {
        ...
        address underlying = address(CErc20Interface(address(cToken)).underlying());
        ...
        return PriceOracleInterface(address(oracles[underlying])).price(underlying);
    }
